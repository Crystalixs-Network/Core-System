...
public final class DefaultHomeStore implements HomeStore {
    ...
    public Optional<HomeModel> findById(long homeId) {
        try {
            return config.query("SELECT * FROM home WHERE id = ?;")
    ...
    public Optional<HomeModel> findByPlayerAndName(UUID playerId, String name) {
        try {
            return config.query("SELECT * FROM home WHERE player_uuid = ? AND name = ?;")
                    .single(call()
    ...
    public List<HomeModel> findByPlayerId(UUID playerId) {
        try {
            return config.query("SELECT * FROM home WHERE player_uuid = ? ORDER BY name;")
    ...
    public void rename(UUID playerId, String oldName, String newName) {
        try {
            boolean changed = config.query("UPDATE home SET name = ? WHERE player_uuid = ? AND name = ?;")
                    .single(call()
                            .bind(newName)
                            .bind(playerId.toString())
                            .bind(oldName)
    ...
    public void updatePosition(long homeId, String worldName, double x, double y, double z, float yaw, float pitch) {
        try {
            boolean changed = config.query("UPDATE home SET world_name = ?, x = ?, y = ?, z = ?, yaw = ?, pitch = ? WHERE id = ?;")
                    .single(call()
                            .bind(worldName)
                            .bind(x)
                            .bind(y)
    ...
    public void updateIcon(long homeId, String icon) {
        try {
            boolean changed = config.query("UPDATE home SET icon = ? WHERE id = ?;")
                    .single(call()
                            .bind(icon)
                            .bind(homeId)
                    )
    ...
    public boolean deleteByPlayerAndName(UUID playerId, String name) {
        try {
            return config.query("DELETE FROM home WHERE player_uuid = ? AND name = ?;")
                    .single(call()
                            .bind(playerId.toString())
                            .bind(name)
                    )
